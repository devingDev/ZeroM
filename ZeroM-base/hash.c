...
void sha256_update(struct sha256_buff* buff, const void* data, unsigned long size) {
    ...
    if (size + buff->chunk_size >= 64) {
        ...
        sceClibMemcpy(tmp_chunk, buff->last_chunk, buff->chunk_size);
        sceClibMemcpy(tmp_chunk + buff->chunk_size, ptr, 64 - buff->chunk_size);
    ...
    sceClibMemcpy(buff->last_chunk + buff->chunk_size, ptr, size);
...
void sha256_finalize(struct sha256_buff* buff) {
    ...
    buff->chunk_size++;
    sceClibMemset(buff->last_chunk + buff->chunk_size, 0, 64 - buff->chunk_size);
    ...
    if (buff->chunk_size > 56) {
        ...
        sceClibMemset(buff->last_chunk, 0, 64);
    }
...
int sceSblDmac5HashTransform(SceSblDmac5HashTransformParam *param, uint32_t command, uint32_t extra){
    hash_it(param->src, param->size, param->dst);
    return 0;
}